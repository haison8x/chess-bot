...
jQuery("#player-bottom").click(function () {
  showHint();
})
...
function showHint() {
  ...
  var move = getBestMove(game, nextColor, globalSum)[0];
  console.log(move)
...
function getCurrentPgn() {
  ...
  jQuery("#scroll-container .move").each(function (i, e) {
    let step = jQuery(e).contents().filter(function () {
      return this.nodeType == Node.TEXT_NODE;
    })[0].nodeValue;

    var tempPgn = "";
    jQuery(e).find("div").each(function (i1, e1) {
      tempPgn += " " + jQuery(e1).text();
    });
    pgn += step + tempPgn + " "
  });
...
function getNextColor() {
  let stepCount = jQuery("#scroll-container .move div").length;
  return stepCount % 2 == 0 ? "w" : "b";
}
